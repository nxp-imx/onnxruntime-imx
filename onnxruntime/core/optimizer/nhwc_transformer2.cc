...
class NhwcTransformerImpl2 {
 public:
  NhwcTransformerImpl2(Graph& graph, std::string provider, const logging::Logger& _logger) noexcept :
...
bool NhwcTransformerImpl2::isNot9x9(Node& node) {
...
NodeIndex NhwcTransformerImpl2::InsertPermuteParentNode(Node& node, const Node::EdgeEnd* edge, bool bNHWC) {
...
NodeIndex NhwcTransformerImpl2::InsertPermuteChildNode(Node& node, bool bNHWC) {
...
bool NhwcTransformerImpl2::RequiresWeightsPermutation(Node& node) {
...
NodeIndex NhwcTransformerImpl2::ReplaceNode(Node& node) {
...
void NhwcTransformerImpl2::PermuteWeights(NodeArg *input_def, NodeArg** nhwc_conv_W_arg, __attribute__ ((unused)) const std::string& execution_provider, bool nodeIsDepthwise) {
...
bool NhwcTransformerImpl2::SuportsReplacementNHWC(Node& node) {
   return
...
DataLayout NhwcTransformerImpl2::RequiredLayout(Node& node) {
...
void NhwcTransformerImpl2::Transform(Node& node) {
...
void NhwcTransformerImpl2::Finalize(__attribute__ ((unused)) bool& modified) {
  modified = false;
}

Status NhwcTransformer2::ApplyImpl(Graph& graph, bool& modified, int graph_level, const logging::Logger& logger) const {
  ...
  if (!provider.empty()) {
    NhwcTransformerImpl2 impl(graph, provider, logger);
