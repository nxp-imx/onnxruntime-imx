...
void NhwcOptimizerTester(const std::function<void(NhwcTestHelper& helper)>& build_test_case,
  ...
  // ToDO: Ignore the test if NHWc is not supported by the platform.
  // check ACL/Arm-NN?
  ...
  Model model("nhwc", false, ModelMetaData(), IOnnxRuntimeOpSchemaRegistryList(), domain_to_version,
  ...
  for (auto& node : model.MainGraph().Nodes()) {
    std::cout << node.OpType() << " " << node.GetExecutionProviderType() << std::endl;
    node.SetExecutionProviderType(kAclExecutionProvider);
  }
  ...
  auto run_model = [&](TransformerLevel level, std::vector<OrtValue>& fetches) {
    ...
    NhwcInferenceSession session{session_options, &DefaultLoggingManager()};
...
//  printTensor(level2_fetches.front().Get<Tensor>());
//  printTensor(level3_fetches.front().Get<Tensor>());
...
/*
TEST(NhwcOptimizerTests, Test01Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 2, 5, 5});
    auto* output_arg = helper.MakeOutput();

    helper.AddConvNode(input_arg, output_arg, {2, 2, 1, 1});
  };
...
TEST(NhwcOptimizerTests, Test02Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 4, 3, 3});
    auto* output_arg = helper.MakeOutput();
...
TEST(NhwcOptimizerTests, Test1Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    ...
    helper.AddConvNode(input_arg, output_arg, {1, 4, 3, 3});
  };
...
/*
TEST(NhwcOptimizerTests, Test2Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 4, 28, 28});
    auto* output_arg = helper.MakeOutput();
...
TEST(NhwcOptimizerTests, Test3Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 4, 3, 3});
    auto* output_arg = helper.MakeOutput();
...
TEST(NhwcOptimizerTests, Test4Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 4, 28, 28});
    auto* output_arg = helper.MakeOutput();

    helper.AddConvNode(input_arg, output_arg, {1, 4, 28, 28});
  };
...
TEST(NhwcOptimizerTests, Test5Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 4, 28, 28});
    auto* output_arg = helper.MakeOutput();

    helper.AddConvNode(input_arg, output_arg, {2, 4, 28, 28});
  };
...
/*
TEST(NhwcOptimizerTests, Test6Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 4, 5, 5});
    auto* output_arg = helper.MakeOutput();
...
TEST(NhwcOptimizerTests, Test7Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 4, 5, 5});
    auto* output_arg = helper.MakeOutput();

    helper.AddConvNode(input_arg, output_arg, {2, 4, 5, 5});
  };
...
/*
TEST(NhwcOptimizerTests, Test8Nhwc) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 4, 9, 9});
    auto* output_arg = helper.MakeOutput();
...
/*
TEST(NhwcOptimizerTests, ConvNchw) {
  auto test_case = [&](const std::string& activation_op_type) {
    ...
    auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
      auto op_to_count = session.CountOpsInGraph();
      EXPECT_EQ(op_to_count["nhwc.Conv"], 1);
      EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 0);
      EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 1);
      if (!activation_op_type.empty()) {
...
TEST(NhwcOptimizerTests, ConvNhwc) {
  auto test_case = [&](const std::string& activation_op_type) {
    auto build_test_case = [&](NhwcTestHelper& helper) {
...
TEST(NhwcOptimizerTests, ConvNhwcGrouped) {
  auto test_case = [&](const std::string& activation_op_type) {
    auto build_test_case = [&](NhwcTestHelper& helper) {
...
TEST(NhwcOptimizerTests, ConvDepthwise) {
  auto test_case = [&](const std::string& activation_op_type) {
    auto build_test_case = [&](NhwcTestHelper& helper) {
...
TEST(NhwcOptimizerTests, ConvPointwise) {
  auto test_case = [&](const std::string& activation_op_type) {
    auto build_test_case = [&](NhwcTestHelper& helper) {
...
TEST(NhwcOptimizerTests, ConvMaxPool) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
  ...
  auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
    auto op_to_count = session.CountOpsInGraph();
    EXPECT_EQ(op_to_count["nhwc.Conv"], 1);
    EXPECT_EQ(op_to_count["nhwc.MaxPool"], 1);
    EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 1);
    EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 1);
  };
...
TEST(NhwcOptimizerTests, ConvMaxPoolDilations) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
...
TEST(NhwcOptimizerTests, ConvAveragePool) {
  auto test_case = [&](bool count_include_pad) {
    ...
    auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
      auto op_to_count = session.CountOpsInGraph();
      EXPECT_EQ(op_to_count["nhwc.Conv"], 1);
      EXPECT_EQ(op_to_count["nhwc.AveragePool"], 1);
      EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 1);
      EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 1);
    };
...
TEST(NhwcOptimizerTests, ConvGlobalPool) {
  auto test_case = [&](const std::string& op_type) {
    ...
    auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
      auto op_to_count = session.CountOpsInGraph();
      EXPECT_EQ(op_to_count["nhwc.Conv"], 1);
      EXPECT_EQ(op_to_count["nhwc." + op_type], 1);
      EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 1);
      EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 1);
    };
...
TEST(NhwcOptimizerTests, ConvAddFusion) {
  auto test_case = [&](const std::string& op_type, int opset_version, bool do_relu) {
    ...
    auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
      auto op_to_count = session.CountOpsInGraph();
      EXPECT_EQ(op_to_count["nhwc.Conv"], 2);
      EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 1);
      EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 1);
    ...
    NhwcOptimizerTester(build_test_case, check_nhwc_graph, opset_version);
  };
...
TEST(NhwcOptimizerTests, ConvNoBiasAddFusion) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    auto* input_arg = helper.MakeInput({1, 32, 28, 28});
    auto* conv1_output_arg = helper.MakeIntermediate();
    auto* conv2_output_arg = helper.MakeIntermediate();
...
TEST(NhwcOptimizerTests, FusedConvAddFusion) {
...
TEST(NhwcOptimizerTests, ConvConcat) {
  auto test_case = [&](int axis, int channel_count, int reorder_output_count) {
    ...
    auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
      auto op_to_count = session.CountOpsInGraph();
      EXPECT_EQ(op_to_count["nhwc.Conv"], 3);
      EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 1);
      EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], reorder_output_count);
    };
...
TEST(NhwcOptimizerTests, ConvReuseWeightsOIHWBiBo) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
  ...
  auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
    ...
    EXPECT_EQ(op_to_count["nhwc.Conv"], 3);
    EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 1);
    EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 3);
    ...
    for (auto& node : graph.Nodes()) {
      if (node.Domain() == kMSNhwcDomain && node.OpType() == "Conv") {
...
TEST(NhwcOptimizerTests, ConvReuseWeightsOIHWBo) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
  ...
  auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
    auto op_to_count = session.CountOpsInGraph();
    EXPECT_EQ(op_to_count["nhwc.Conv"], 4);
    EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 2);
    EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 4);
...
TEST(NhwcOptimizerTests, ShapeInferencing) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    ONNX_NAMESPACE::TypeProto type_proto;
    type_proto.mutable_tensor_type()->set_elem_type(ONNX_NAMESPACE::TensorProto_DataType_FLOAT);
  ...
  auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
    auto op_to_count = session.CountOpsInGraph();
    EXPECT_EQ(op_to_count["nhwc.Conv"], 3);
    EXPECT_EQ(op_to_count["nhwc.MaxPool"], 2);
    EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 0);
    EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 1);
    EXPECT_EQ(op_to_count["Add"], 0);
  };
...
TEST(NhwcOptimizerTests, ShapeInferencing2) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
    ONNX_NAMESPACE::TypeProto type_proto;
    type_proto.mutable_tensor_type()->set_elem_type(ONNX_NAMESPACE::TensorProto_DataType_FLOAT);
    type_proto.mutable_tensor_type()->mutable_shape()->add_dim()->set_dim_value(1);
...
TEST(NhwcOptimizerTests, MixedOutputUsage) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
  ...
  auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
    ...
    EXPECT_EQ(op_to_count["nhwc.Conv"], 2);
    EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 0);
    EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 2);
  };
...
TEST(NhwcOptimizerTests, TensorAlignment) {
  auto build_test_case = [&](NhwcTestHelper& helper) {
  ...
  auto check_nhwc_graph = [&](NhwcInferenceSession& session) {
    ...
    EXPECT_EQ(op_to_count["nhwc.Conv"], 0);
    EXPECT_EQ(op_to_count["nhwc.MaxPool"], 0);
    EXPECT_EQ(op_to_count["nhwc.ReorderInput"], 0);
    EXPECT_EQ(op_to_count["nhwc.ReorderOutput"], 0);
  };
